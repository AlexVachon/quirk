...
void Sema::checkFunction(FunctionNode* f) {
    ...
    // Default void return if inferred as auto
...
void Sema::checkVarDecl(VarDeclNode* node) {
    ...
    if (auto lit = dynamic_cast<LiteralNode*>(node->lhs.get())) {
        ...
        if (!node->typeAnnotation.empty() && finalType != "any" &&
            ...
            // Allow Auto-boxing: String = "literal"
            if (finalType == "String" &&
                (exprType == "cstring" || exprType == "string"))
                valid = true;

            // Allow int <-> ptr (unsafe)
        ...
        checkExpression(binOp->left.get());  // Just check existence
    }
}
...
void Sema::checkFor(ForNode* node) {
    ...
    if (iterType == "cstring" || iterType == "string")
        ...
        // Look up __iter -> __next return type
...
std::string Sema::checkConstructor(ConstructorNode* node) {
    return structRegistry.count(node->structName) ? node->structName
                                                  : "unknown";
}
...
std::string Sema::checkCall(CallNode* node) {
    ...
    if (auto l = dynamic_cast<LiteralNode*>(node->callee.get())) {
        if (l->value == "print") {
            std::cerr << "[SEMA] Found 'print' call. Arg Count: "
    ...
    for (auto& a : node->args) {
        std::string argType = checkExpression(a.value.get());

        // ---- [DEBUG LOG START] ----
        if (auto l = dynamic_cast<LiteralNode*>(node->callee.get())) {
            if (l->value == "print") {
                std::cerr << "  [SEMA] Arg Type: " << argType << std::endl;
            }
        }
        // ---- [DEBUG LOG END] ----
    }
...
std::string Sema::resolveVariable(const std::string& name) {
    ...
    if (name == "print" || name == "printf" || name == "free" || name == "exit")
        return "void";
    if (name == "char_at")
        return "char";  // Returns a character (byte)
    if (name == "set_char_at")
        return "void";
    ...
    if (!currentClass.empty()) {
        ...
        if (methodRegistry[currentClass].count(staticName)) {
            std::string ret =
                methodRegistry[currentClass][staticName]->returnType;
            return ret.empty() ? "void" : ret;
        }
    }
...
void Sema::checkWith(WithNode* node) {
    ...
    // Ensure __enter and __exit exist
...
void Sema::checkReturn(ReturnNode* node) {
    std::string actual =
        node->expression ? checkExpression(node->expression.get()) : "void";
    ...
    if (target == "auto") {
        ...
        if (!currentFunctionNode->cls.empty())
            methodRegistry[currentFunctionNode->cls][currentFunctionNode->name]
                ->returnType = actual;
        return;
    }
    ...
    if (target != actual && target != "any") {
        bool targetIsPtr =
            (target == "ptr" || target == "cstring" || target == "string");
        bool actualIsPtr =
            (actual == "ptr" || actual == "cstring" || actual == "string");
        ...
        // 2. [FIX] Allow returning 0 (int) as NULL for Pointers/Structs
        if (actual == "int" &&
            (targetIsPtr || structRegistry.count(target) || target == "any")) {
            valid = true;
        }

        // 3. [FIX] Allow Auto-boxing (returning "literal" for String function)
        ...
        if (!valid) {
            std::cerr << "Error: Function " << currentFunctionNode->name
                      << " expected " << target << " but got " << actual
                      << std::endl;
            exit(1);
        }
    }
