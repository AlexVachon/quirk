...
// ─── Stdlib symbol index (generated by `make docs` in the compiler) ──
//
// Map of bare-identifier → array of candidate definitions:
//   {
//     "exists": [{ package: "fs", qualified: "exists",
//                  signature: "extern define exists(path: String) -> Bool",
//                  doc: "...", kind: "define" }],
//     "length": [ ... multiple structs ... ]
//   }
//
// Loaded lazily once per extension activation. Used as a fallback in
// `provideHover` when `executeDefinitionProvider` can't navigate to a
// stdlib symbol — most commonly when the cursor is on `argv` in
// `sys.argv()` and `argv` isn't separately `use`d.
...
let _stdlibIndex: Record<string, StdlibSymbol[]> | null = null;
let _stdlibIndexAttempted = false;
function loadStdlibIndex(): Record<string, StdlibSymbol[]> {
    if (_stdlibIndex || _stdlibIndexAttempted) return _stdlibIndex || {};
    _stdlibIndexAttempted = true;
    try {
        // Bundled alongside compiled extension code: `<ext>/data/stdlib-index.json`.
        const candidates = [
            path.join(__dirname, '..', 'data', 'stdlib-index.json'),
            path.join(__dirname, 'data', 'stdlib-index.json'),
        ];
        for (const c of candidates) {
            if (fs.existsSync(c)) {
                _stdlibIndex = JSON.parse(fs.readFileSync(c, 'utf-8'));
                return _stdlibIndex!;
            }
        }
...
function renderStdlibHover(word: string, candidates: StdlibSymbol[]): vscode.Hover {
    const md = new vscode.MarkdownString();
    md.isTrusted = true;
    // Group by kind label, then signature, then doc. When multiple
    // candidates exist (e.g. `length` on List, Map, String), each
    // gets its own section so the user can see them all and pick.
    for (let i = 0; i < candidates.length; i++) {
        const c = candidates[i];
        if (i > 0) md.appendMarkdown('\n\n---\n\n');
        md.appendMarkdown(`\`(${c.kind}) ${c.qualified}\` — *stdlib ${c.package}*\n\n`);
        md.appendCodeblock(c.signature.replace(/\s*\{$/, ''), 'quirk');
        if (c.doc && c.doc.trim()) {
            md.appendMarkdown('\n');
            md.appendMarkdown(c.doc);
        }
    }
...
export class QuirkHoverProvider implements vscode.HoverProvider {
    public async provideHover(
        ...
        // Stdlib index fallback (v0.2.12). `executeDefinitionProvider`
        // works when the symbol is reachable via an explicit
        // `from X use { Y }` import, but module-alias accesses like
        // `sys.argv()` land on `argv` without a navigable definition.
        // The pre-built JSON index lets us still surface the signature
        // + docstring for any stdlib symbol the user types.
        const stdlib = loadStdlibIndex();
        if (word in stdlib && stdlib[word].length > 0) {
            return renderStdlibHover(word, stdlib[word]);
        }
